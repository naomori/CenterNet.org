...
ANN_PATH = '../../data/arc/annotations/'
IMG_PATH = '../../data/arc/'
ANN_FILES = {'train': 'train_arc.json',
             'val': 'val_arc.json'}
DEBUG = False
RESIZE = True

class_name = [
    '__background__', 'Binder', 'Balloons', 'Baby_Wipes', 'Toilet_Brush',
    'Toothbrushes', 'Crayons', 'Salts', 'DVD', 'Glue_Sticks', 'Eraser',
    'Scissors', 'Green_Book', 'Socks', 'Irish_Spring', 'Paper_Tape',
    'Touch_Tissues', 'Knit_Gloves', 'Laugh_Out_Loud_Jokes', 'Pencil_Cup',
    'Mini_Marbles', 'Neoprene_Weight', 'Wine_Glasses', 'Water_Bottle',
    'Reynolds_Pie', 'Reynolds_Wrap', 'Robots_Everywhere', 'Duct_Tape',
    'Sponges', 'Speed_Stick', 'Index_Cards', 'Ice_Cube_Tray', 'Table_Cover',
...
def count_agnostic(split):
    ...
    for img_id in images:
        ...
        for ann in anns:
            bbox = ann['bbox']
            center = (
                (bbox[0] + bbox[2] / 2) // 4, (bbox[1] + bbox[3] / 2) // 4)
            for c in centers:
...
def count(split):
    ...
    for img_id in images:
        ...
        for ann in anns:
            ...
            center = (
            (bbox[0] + bbox[2] / 2) // 4, (bbox[1] + bbox[3] / 2) // 4,
            ann['category_id'], bbox)
            for c in centers:
                if center[0] == c[0] and center[1] == c[1] and \
                    center[2] == c[2] and iou(_coco_box_to_bbox(bbox),
                                              _coco_box_to_bbox(c[3])) < 2: # 0.5:
                    cnt += 1
                    if DEBUG:
...
def count_iou(split):
    ...
    for img_id in images:
        ...
        for ann in anns:
            if ann['iscrowd'] > 0:
                continue
            bbox = _coco_box_to_bbox(ann['bbox']).tolist() + [
                ann['category_id']]
            for b in bboxes:
                if iou(b, bbox) > 0.5 and b[4] == bbox[4]:
                    cnt += 1
                    if DEBUG:
                        file_name = coco.loadImgs(ids=[img_id])[0][
                            'file_name']
                        img = cv2.imread(
                            '{}/{}2017/{}'.format(IMG_PATH, split,
                                                  file_name))
            ...
            for t, gt in enumerate(g):
                ...
                for k in range(len(anchors)):
                    if ious[k, t] > 0.5:
                        ...
                        cl = (np.array([255, 0, 0]) * ious[k, t]).astype(
                            np.int32).tolist()
                        cv2.rectangle(img, (x1, y1), (x2, y2), cl, 1,
                                      cv2.LINE_AA)
            cv2.imshow('img', img)
            cv2.waitKey()
        miss = 0
